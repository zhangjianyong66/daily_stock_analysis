...
- Agent 配置（24 字段）：问股生成方式、Agent 模式、最大推理步数、自动匹配兜底策略、策略列表、策略目录、自然语言路由、架构、编排器模式、超时、风险否决、Deep Research 预算/超时、记忆、策略自动权重、策略路由、问股可见对话上下文压缩、事件监控开关/间隔、告警规则 JSON。普通用户只看到新的 `AGENT_BACKEND` 选择器；旧 `AGENT_GENERATION_BACKEND` 仅保留配置兼容。
...
- 数据源与搜索：TickFlow、SerpAPI、Brave、Bocha、MiniMax、SearXNG 公共实例开关、ETF Anspire 双查询、BIAS 阈值和 Pytdx 服务器字段。
