...
- 日资金流按交易日选最新行，并计算最近/前序 3 日、最近 5 日和 10 日累计；`as_of` 会随数据传入报告，供分析时判断时效。
- 逐笔数据明确提供买盘、卖盘、中性盘分类时，报告可展示盘中主动买入、卖出和净流入估算；中性盘单列，未知方向不归类。盘中估算仅作普通参考。
- ETF 继续使用系统通用分析方式。资金流不会通过 ETF 专用规则强制修改评分、买卖动作、仓位、止损止盈或持有期限。
- 流动性信息用于提示限价单、滑点与折溢价风险，不作为独立交易结论。
