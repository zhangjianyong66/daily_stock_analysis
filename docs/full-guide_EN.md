...
| `DEFAULT_ANALYSIS_SKILL` | One server-side default strategy ID for new stock analyses without an explicit selection; leave empty to resolve through `AGENT_SKILLS` and then the built-in default | Empty | No |
| `AGENT_SKILLS` | Legacy comma-separated default strategy list; used only when `DEFAULT_ANALYSIS_SKILL` is empty or unavailable | Empty | No |
...
> Note: `POST /api/v1/analysis/analyze` accepts `skills` as an array of strategy IDs. If omitted, the server resolves `DEFAULT_ANALYSIS_SKILL`, then `AGENT_SKILLS`, then the built-in default. The saved single default applies to Home, batch/scheduled analysis, and Ask Stock, but not market review. The legacy field `strategies` is still accepted for backward compatibility.
...
> Note: The Web Home selector separates per-analysis selection from the saved default. It does not send `skills` before the user changes the selection; the star action saves `DEFAULT_ANALYSIS_SKILL` through system settings. Default changes affect only newly created tasks, while queued/running tasks and historical reports keep their frozen strategy snapshot.
