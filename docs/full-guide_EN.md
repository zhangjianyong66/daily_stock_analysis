...
| `DEFAULT_ANALYSIS_SKILL` | One fallback strategy ID used when the latest complete daily bars cannot produce a reliable match; leave empty to resolve through `AGENT_SKILLS` and then the built-in default | Empty | No |
| `AGENT_SKILLS` | Legacy comma-separated fallback strategy list; used after auto-match failure only when `DEFAULT_ANALYSIS_SKILL` is empty or unavailable | Empty | No |
...
> Behavior note: Search and social sentiment are optional enhancements and failures do not block the technical-analysis flow. A single-market review merges its three topics into one search (logical limit 6, provider `top_k=12`). With Anspire configured, ETF intelligence still uses at most two physical requests for 3-day events and 30-day analysis; standard non-ETF analysis preserves each market's existing five dimensions and merges them into a recent-events request and a 180-day analysis request, each with `top_k=18` (A-shares retain announcements, while foreign stocks retain industry analysis). ETF group failures do not fall back. For ordinary stocks, only a physically failed group uses the existing non-Anspire provider fallback; a successful empty group does not fan out. Trusted results use process-wide cross-instance caching and singleflight, with a 15-minute event TTL and 6-hour analysis TTL, so cache hits do not count as physical calls. The six-dimension Agent tool path remains unchanged. Results must still pass identity, date, junk-page, and deterministic-routing admission. If every dimension is empty, no report text, `news_context`, or `news_intel` row is produced. Public SearXNG discovery is disabled by default.
...
| `DATA_SOURCE_REALTIME_TIMEOUT_SECONDS` | Additional per-provider realtime wait limit. `0` removes only this configurable cap; Tencent/Sina remain capped at 10 seconds, bulk sources at 8 seconds, and the full route at 20 seconds. With the default ordering, Sina starts as a hedge when Tencent is still pending after 5 seconds; a positive value can only tighten each source wait. | `12` | Optional |
...
> **Behavior Notes:**
> - A-share ETFs now route to the actual Tencent, Sina, and Eastmoney implementations named in `REALTIME_SOURCE_PRIORITY`. `efinance` and `akshare_em` share the Eastmoney physical upstream, so a network-level failure blocks duplicate requests to that upstream for the current quote attempt. When every live source fails, only a same-market-day last-good quote no older than 30 minutes may be returned and it is explicitly marked `stale`.
> - **A-shares**: Returns aggregated capabilities by `valuation/growth/earnings/institution/capital_flow/dragon_tiger/boards`.
> - **A-share exchange-traded ETFs**: `capital_flow` routes by Shanghai/Shenzhen code to Eastmoney daily main/large/super-large flow. A display-only intraday active-flow estimate is attached only when trades have an explicit vendor buy/sell/neutral classification. Missing stock-only capabilities remain `not_supported`, and every flow failure is fail-open.
...
- **Strategy selection** - The Home page supports explicit per-analysis strategy selection and can set a deployment default or restore system-default resolution from the same menu; the effective default is shown without sending explicit `skills` until the user changes it
...
| `/api/v1/usage/search/dashboard` | GET | Search-call totals, provider/key/source breakdowns, faults, audit health, and server-side paginated rows; supports `today|7d|month|all|custom` and provider/source/status/error/key filters |
| `/api/v1/usage/search/faults` | GET | Return active search-provider faults, normal/degraded/unavailable provider status, and audit health for the global Web banner |
| `/api/v1/usage/search/calls/{id}` | GET | Return one call's complete redacted request/response details; requires enabled admin auth and a logged-in administrator |
| `/api/v1/usage/search/export.csv` | GET | Export all filtered reconciliation summary rows without snapshots; requires enabled admin auth and a logged-in administrator |
| `/api/v1/usage/search/calls/{id}/export.json` | GET | Download one call's audit metadata and redacted payloads; requires enabled admin auth and a logged-in administrator |
...
> Note: `POST /api/v1/analysis/analyze` accepts `skills` as an array of concrete strategy IDs and executes an explicit selection directly. When omitted, Home, batch, CLI, and scheduled stock analyses first perform deterministic matching from the latest valid complete daily bars. Insufficient or stale bars, unavailable calendars, unclear patterns, and unavailable candidates fall back to the frozen `DEFAULT_ANALYSIS_SKILL`, then `AGENT_SKILLS`, then the built-in default. Ask Stock and market review do not use daily-bar auto matching. The legacy `strategies` alias remains supported.
...
> Note: The Web Home selector defaults to “Auto match” and omits `skills`; choosing a concrete strategy sends a single ID for that run. The star action saves `DEFAULT_ANALYSIS_SKILL` as the fallback used only when auto matching fails. Reanalysis always follows the current menu selection, so Auto match evaluates the latest eligible bars instead of reusing the historical effective strategy. Queued/running tasks retain their enqueue-time fallback and catalog, while historical reports read only the persisted execution snapshot.
...
> Search-call accounting: `search_api_calls` writes one row for every real outbound HTTP request, including automatic retries, alternate keys, provider fallbacks, and SearXNG instance attempts. Cache hits, singleflight waiters, database reads, local filtering, article-body fetching, and public-instance directory refreshes are not counted. The healthy path records one call for a market-review search, at most two for a cold ETF analysis, and at most two for standard non-ETF Anspire analysis; real failure fallbacks are counted separately. SQLAlchemy creates the tables at startup; old text logs are not backfilled.
