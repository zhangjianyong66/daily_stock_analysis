...
#1381 在 AnalysisContextPack 之外新增一个小型每日大盘环境摘要通道，避免把 `market_review` / `market_light` 直接 pack 化。`DAILY_MARKET_CONTEXT_ENABLED` 默认开启；当 `MARKET_REVIEW_ENABLED=true` 且 `DAILY_MARKET_CONTEXT_ENABLED=true` 时，`StockAnalysisPipeline` 会按个股市场（`cn` / `hk` / `us` / `jp` / `kr`）加载每日大盘上下文：优先复用 `analysis_history(code=MARKET, report_type=market_review)` 中目标交易日、同市场、同报告语言的记录；没有匹配记录时才调用 `run_market_review(..., return_structured=True, send_notification=False)` 生成本次上下文，且通过进程内 cache 与 market review lock 避免同一目标交易日重复生成。内部快照字段 `daily_market_context_target_date` 保存 Pipeline 计算的有效交易日，与公开 `market_review_payload.date` 的报告生成日以及 `generated_at` / 历史 `created_at` 的实际生成时间相互独立；旧记录缺少该字段时继续按 payload 日期或创建日期精确匹配，不做跨日猜测。`DAILY_MARKET_CONTEXT_ENABLED=false` 只关闭个股分析的低敏摘要注入与保守护栏，不关闭大盘复盘本身。
...
| `missing` | 当前路径需要该字段，但未请求、未进入输入或实际为空，且没有本轮抓取失败证据。 | 实时行情功能关闭；DB 无最近日线。 |
| `not_supported` | 当前市场、数据源或路径不支持该字段，不应误报为错误。 | 某些市场无筹码分布或资金流。 |
| `fallback` | 首选来源不可用，使用了备用来源或旧路径。 | 持仓价格从实时行情 fallback 到历史收盘价。 |
| `stale` | 字段存在，但时间新鲜度不足。 | 实时源全部失败后使用同交易日 30 分钟内的 last-good 行情；持仓估值中的 `price_stale` / `fx_stale`。 |
| `estimated` | 字段是估算值，不应当作完整事实。 | 盘中用实时价补今日 bar 后生成技术估计。 |
| `partial` | 数据块部分可用、部分缺失。 | 大盘红绿灯 `data_quality=partial` 或工具返回 `partial_cache`。 |
| `fetch_failed` | 当前路径确认尝试过抓取，但本次抓取失败。 | 实时 provider 均失败且无合格 last-good；`fundamental_context.status == "failed"`。 |
