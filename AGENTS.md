...
- 日资金流必须按日期取最新行，并自行计算最近/前序 3 日、5 日和 10 日完整窗口；兼容字段 `main_net_inflow/inflow_5d/inflow_10d` 不得删除，并保留 `as_of` 供报告判断数据时效。
- 盘中主动流只可从逐笔供应商买盘/卖盘/中性盘分类聚合，中性与未知方向不得归入净流入；盘中估算只作参考，汇总成交量额或价格涨跌不得用于猜测资金方向。日流与盘中流必须拆分调用，日流先返回，盘中只使用剩余预算且最多 3 秒；盘中超时不得丢失日流。
- ETF 与普通标的一样走通用分析和市场环境护栏。资金流可以作为普通参考，但不得通过 ETF 专用规则强制修改评分、买卖动作、仓位、止损止盈或持有期限。
- ETF 流动性信息用于限价单、滑点和折溢价风险提示；资金流单源失败保持 fail-open。
- 目标回归：`.venv/bin/python -m pytest tests/test_etf_capital_flow.py tests/test_fundamental_context.py tests/test_decision_stability.py tests/test_analyzer_news_prompt.py tests/test_data_tools_get_capital_flow.py tests/test_daily_market_context_guardrail.py -q`。
...
- HTTP 2xx 搜索响应只能从顶层错误字段和 `base_resp` 错误元数据识别余额、认证、权限或限流语义；不得扫描正常结果集合或完整响应正文，否则结果内容中的同名业务词会误激活供应商故障。HTTP 非 2xx 仍可检查完整错误正文。
...
- 最终交付、提交或创建 / 更新 PR 前，本地默认只执行覆盖实际改动面及直接上下游契约的定向验证，不机械执行完整门禁；交付说明必须列出实际执行项、未验证项和对应风险。
- 涉及公共配置、API / Schema、认证、调度、数据源 fallback、报告结构、共享基础模块时，应扩大相关定向回归范围。若仍无法可靠判断影响范围，必须先说明风险并征求用户同意，再决定是否在本地执行完整门禁。
- GitHub CI 是默认的全量测试兜底；除非用户明确要求或同意，Codex 不在本地自动执行 `./scripts/ci_gate.sh`。CI 未运行、未覆盖改动面或阻断检查失败时，不得把本地定向测试描述为完整门禁通过。
...
- Python 后端改动：
  ...
  - 本地默认不执行完整 `./scripts/ci_gate.sh`；按改动文件、现有测试映射、业务契约和影响面选择受影响测试，并覆盖直接上下游回归
  - 最低要求：对改动 Python 文件执行 `python -m py_compile <changed_python_files>`，并运行受影响的 pytest 用例或测试文件
  - 完整 `./scripts/ci_gate.sh` 默认由 GitHub CI 执行；仅在用户明确要求，或影响范围无法可靠判断且用户同意后，才在本地执行
...
- 报告 / Prompt / 通知：
  ...
  - 个股分析策略统一按“单次明确选择 > 最近完整日 K 线自动匹配 > `DEFAULT_ANALYSIS_SKILL` > `AGENT_SKILLS` > 内置 metadata 默认”解析；问股与大盘复盘不进入日 K 线自动匹配。自动匹配只使用最近已完成交易日、至少 10 根且字段完整的日线，一次只执行一个确定性 winner，失败时使用入队/运行开始时冻结的兜底状态和策略目录。
  - 自动匹配结果必须按股票局部传递，不得改写共享 Pipeline/Analyzer、配置或后续任务；形态报告附件与实际策略快照必须复用同一次日线加载和识别。`strategy_execution.selection_context` 保存截止日、形态、候选与兜底原因，历史/API/Web/Markdown/通知只读该快照。
  - `DEFAULT_ANALYSIS_SKILL` 保存值只接受当前可调用单值，失效时可解释回退；首页默认显示“自动匹配”并省略 `skills`，问股仍按原默认策略范围处理。异步任务在入队时冻结兜底结果和目录，配置热更新只影响后续任务。
