...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.DATA_SOURCE_STOCK_NAME_TIMEOUT_SECONDS': {
    title: '股票名称源超时',
    summary: '限制单个数据源查询股票名称的等待时间。',
    usage: '单位为秒；设为 0 可关闭 manager 层超时兜底。',
    valueNotes: ['超时后会记录失败并尝试后续 provider。'],
    impact: ['影响分析前名称解析和运行流中的 provider fallback。'],
    notes: ['provider 内部已有请求级重试会保留，manager 层不会立即重复调用同一悬挂 provider。'],
  },
  'settings.data_source.DATA_SOURCE_DAILY_TIMEOUT_SECONDS': {
    title: '日线数据源超时',
    summary: '限制单个 provider 获取日线数据的等待时间。',
    usage: '单位为秒；设为 0 可关闭 manager 层超时兜底。',
    valueNotes: ['超时后会切换到下一个可用日线 provider。'],
    impact: ['影响普通分析主路径、回测补数和日线 fallback 速度。'],
    notes: ['该配置不会强杀底层线程，只停止当前调用链路继续等待。'],
  },
  'settings.data_source.DATA_SOURCE_REALTIME_TIMEOUT_SECONDS': {
    title: '实时行情源超时',
    summary: '限制单个 provider 获取实时行情的等待时间。',
    usage: '单位为秒；设为 0 可关闭 manager 层超时兜底。',
    valueNotes: ['超时后同一请求链路会跳过该 provider 的后续实时源别名并继续 fallback。'],
    impact: ['影响现价、盘中技术指标和实时行情降级速度。'],
    notes: ['适合外部行情库偶发卡住时防止分析任务长期等待。'],
  },
  ...
  'settings.system.ANALYSIS_TASK_TIMEOUT_SECONDS': {
    title: '分析任务超时',
    summary: '限制普通分析和后台任务在队列中的最大运行时长。',
    usage: '单位为秒；超过后任务会标记为 failed 并释放重复提交锁。设为 0 可关闭队列级超时兜底。',
    valueNotes: ['默认 1200 秒，适合覆盖常规分析耗时并兜住异常卡住任务。'],
    impact: ['影响任务状态、任务流和同一股票重复提交拦截。'],
    notes: ['Python 无法强制杀死已卡住的工作线程；该配置保证用户可见任务状态不会无限停留在执行中。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.DATA_SOURCE_STOCK_NAME_TIMEOUT_SECONDS': {
    title: 'Stock Name Source Timeout',
    summary: 'Limits how long a single provider may spend resolving a stock name.',
    usage: 'Value is in seconds. Set 0 to disable manager-level timeout protection.',
    valueNotes: ['On timeout, the manager records the failure and tries the next provider.'],
    impact: ['Affects pre-analysis name resolution and provider fallback diagnostics.'],
    notes: ['Provider-level retries remain intact; the manager does not immediately repeat a hanging provider call.'],
  },
  'settings.data_source.DATA_SOURCE_DAILY_TIMEOUT_SECONDS': {
    title: 'Daily Data Source Timeout',
    summary: 'Limits how long a single provider may spend fetching daily bars.',
    usage: 'Value is in seconds. Set 0 to disable manager-level timeout protection.',
    valueNotes: ['On timeout, routing moves to the next available daily-data provider.'],
    impact: ['Affects the main analysis path, daily-data fallback, and backtest data refill speed.'],
    notes: ['This does not kill the underlying Python thread; it stops the current call chain from waiting forever.'],
  },
  'settings.data_source.DATA_SOURCE_REALTIME_TIMEOUT_SECONDS': {
    title: 'Realtime Source Timeout',
    summary: 'Limits how long a single provider may spend fetching realtime quotes.',
    usage: 'Value is in seconds. Set 0 to disable manager-level timeout protection.',
    valueNotes: ['After a timeout, the same request skips later realtime aliases for that provider and continues fallback.'],
    impact: ['Affects current price, intraday indicators, and realtime fallback speed.'],
    notes: ['Useful when external quote libraries occasionally hang.'],
  },
  'settings.data_source.realtime_quotes': {
  ...
  'settings.agent.DEFAULT_ANALYSIS_SKILL': {
    ...
    notes: ['Market review uses its own strategy and is not affected.'],
  },
  'settings.agent.AGENT_SKILLS': {
  ...
  'settings.system.ANALYSIS_TASK_TIMEOUT_SECONDS': {
    title: 'Analysis Task Timeout',
    summary: 'Limits the maximum runtime for normal analysis and background tasks in the queue.',
    usage: 'Value is in seconds. After the limit, the task is marked failed and its duplicate-submission lock is released. Set 0 to disable queue-level timeout protection.',
    valueNotes: ['The default is 1200 seconds, long enough for normal analysis while still catching stuck work.'],
    impact: ['Affects task status, task flow, and duplicate submission checks for the same stock.'],
    notes: ['Python cannot force-kill a stuck worker thread; this setting keeps user-visible task state from staying in processing forever.'],
  },
  'settings.system.ANALYSIS_DELAY': {
