...
def test_etf_uses_general_decision_stability_with_capital_flow_reference() -> None:
    result = _result(
        code="159865",
        decision_type="buy",
        operation_advice="买入",
    ...
    stabilize_decision_with_structure(
        result,
        SimpleNamespace(support_levels=[30.0], resistance_levels=[34.0]),
        _fund_flow(main=-800_000, five_day=-1_200_000, ten_day=-2_000_000),
    )

    assert all("etf" not in key for key in result.dashboard)
    assert result.decision_type == "hold"
    assert result.operation_advice == "震荡观望"
    assert result.dashboard["decision_stability"]["capital_flow_bias"] == "outflow"
