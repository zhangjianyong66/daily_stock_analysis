...
- [ ] AC1: 未配置 Vision 时，图片导入提示前往现有 `VISION_MODEL` 设置；配置文本分析模型但未配置视觉模型时不会误调用文本模型。
- [ ] AC2: 原有 `/api/v1/stocks/extract-from-image` 行为和测试保持兼容。
- [ ] AC3: 用户可上传 1-5 张持仓截图，校对后向空 `cn/CNY` 账户原子写入期初买入；已有交易的账户被拒绝且无数据变化。
- [ ] AC4: 持仓截图中“持仓”和“可用”不相等时使用持仓数量；顶部资金数据不写入现金账本。
- [ ] AC5: 用户可上传 1-5 张实际成交截图，补齐批次日期、编辑时间/方向/数量/价格/费用后向已有账户增量写入。
- [ ] AC6: 委托、撤单、未成交或缺少关键字段的行不能提交。
- [ ] AC7: `trade_time` 可空兼容旧库、旧 API 数据、手工录入和 CSV；有值时交易列表按日期和时间展示。
- [ ] AC8: 成交编号重复或可见指纹重复会跳过；跨图歧义必须人工解决，合法同秒分笔可保留。
- [ ] AC9: 增量批次任一新交易超卖、非法或写入冲突时全部新增交易回滚，已有数据不受影响。
- [ ] AC10: 原图、base64、模型原始响应和完整资产明细不进入数据库或普通日志。
- [ ] AC11: 后端离线测试、Web 组件测试、Lint 和构建通过；真实 Vision 在线调用未执行时在交付中明确说明。
