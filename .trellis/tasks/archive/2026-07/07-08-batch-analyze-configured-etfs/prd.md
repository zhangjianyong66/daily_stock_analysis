...
- [x] 首页能看到配置标的批量分析按钮或等价入口。
- [x] 点击入口后，会批量提交 `STOCK_LIST` / 自选队列中所有非空编码的异步分析任务。
- [x] 普通股票和 ETF 都会被纳入批量提交。
- [x] 当没有配置编码时，页面提示用户当前没有可批量分析的配置标的。
- [x] 当部分编码已有进行中任务时，页面能体现 accepted / duplicate 的结果，不误报为全部失败。
- [x] 相关前端单元测试覆盖成功提交、空配置、重复任务响应至少三个路径。

## Verification

- [x] `cd apps/dsa-web && npm test -- HomePage.test.tsx`
- [x] `cd apps/dsa-web && npm run lint`
- [x] `cd apps/dsa-web && npm run build`
