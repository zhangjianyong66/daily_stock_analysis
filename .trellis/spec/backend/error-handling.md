...
- `data_provider.base.DataFetcherManager._call_fetcher_method(..., timeout_seconds: Optional[float], capability: str, call_scope: Optional[str])`
...
- `data_provider.base.RealtimeSourcePlan(route_source, physical_source, fetcher_name, request_code, kwargs, timeout_seconds, max_attempts, lightweight, hedge_after_seconds)`
- `src.services.run_diagnostics.record_provider_run_started(..., route_source: Optional[str], physical_source: Optional[str])`
...
- 实时行情固定安全上限：腾讯/新浪轻量单标的源单次 `10` 秒、全量源单次 `8` 秒、单只标的整链路 `20` 秒。`DATA_SOURCE_REALTIME_TIMEOUT_SECONDS` 只可进一步收紧单次等待；`0` 表示不额外收紧，不能放大或关闭固定上限。
- 当计划中腾讯紧邻并位于新浪之前时，腾讯等待 `5` 秒仍未完成会启动新浪并行 hedge；腾讯快速失败、空数据或无有效价格时立即启动新浪。首个有效 quote 是 winner，调用返回后的迟到结果不得覆盖 winner、追加持久化 provider run 或写入 last-good。
- fetcher 调用锁和单槽占用的键为 `(id(fetcher), call_scope)`；实时行情使用 `physical_source` 作为 scope。腾讯遗留线程只占用腾讯槽，新浪可独立启动；同一物理源槽被占用时不得再生成新的等待 worker。
- `AkshareFetcher` 的轻量实时限速时间戳和锁按腾讯/新浪 source key 隔离；历史数据与其他旧路径继续使用默认限速 scope。
- 并发 provider 的 started/completed pending key 必须包含 `route_source` 与 `physical_source`；腾讯和新浪虽然共用 `AkshareFetcher`，诊断节点不得只按 provider/operation 配对而发生串线。
...
- 腾讯 5 秒 soft deadline -> 启动新浪 hedge，不把腾讯 worker 强杀；谁先返回有效 quote 谁成为 winner。
- 实时行情 manager hard timeout -> 仅阻断相同物理源，不启动同源重复后台调用；独立物理源继续在总预算内 fallback。
...
- Good：ETF 腾讯超过 5 秒仍运行时新浪实际并行启动并先成功，返回 `source=akshare_sina`、`fallback_from=tencent`；诊断包含两个 started 事件和新浪 winner provider run。
...
- Bad：按 `id(fetcher)` 共享实例级 timeout/锁，使腾讯超时后共用 `AkshareFetcher` 的新浪被静默跳过或继续等待腾讯遗留线程。
...
- 实时行情测试必须断言：ETF Tencent/Sina/EM 真路由、轻量源最多两次总尝试、空/无效 quote 不重试、10/5/8/20 秒契约、腾讯与新浪物理锁/限速隔离、同物理上游网络失败去重、解析错误可换客户端、主行情不被补充失败丢弃。
- hedge 测试必须断言：腾讯仍运行时新浪已实际启动、快速失败无需等待 5 秒、同源遗留调用不生成新 worker、迟到 loser 不覆盖 winner/last-good、provider diagnostics 能识别新浪 winner。
...
plan = RealtimeSourcePlan(
    ...
    max_attempts=2,
    lightweight=True,
    hedge_after_seconds=5.0,
)

quote = manager._call_fetcher_method(
    fetcher,
    "get_realtime_quote",
    "159869",
    timeout_seconds=10.0,
