...
def get_stock_index_metadata(stock_code: str) -> StockIndexMetadata | None:
    """Return lightweight metadata for an indexed symbol when available."""
    code = str(stock_code or "").strip()
    if not code:
        return None
    ...
    if not name:
        return None
    upper = code.upper()
    market = "cn" if upper.startswith(("SH", "SZ", "CSI")) or upper.endswith((".SH", ".SZ", ".CSI")) else ""
    asset_type = "index" if upper.startswith(("SH", "SZ", "CSI")) or upper.endswith((".SH", ".SZ", ".CSI")) else "stock"
    return StockIndexMetadata(code, code, name, (code,), market, asset_type)
