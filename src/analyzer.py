...
"""
        ...
        if has_capital_flow:
            ...
            if is_cn_etf_symbol(code):
                ...
                prompt += f"""
### 场内 ETF 二级市场交易资金流（普通参考）
| 指标 | 数值 | 决策含义 |
|------|------|----------|
| 日资金流截止日 | {stock_flow.get('as_of', 'N/A')} | 与完整日线 {effective_daily_bar_date} 对照判断数据时效 |
| 主力净流入 / 净占比 | {stock_flow.get('main_net_inflow', 'N/A')} / {stock_flow.get('main_net_inflow_pct', 'N/A')}% | 比例为主、金额为辅 |
| 前一日主力净流入 / 净占比 | {stock_flow.get('previous_main_net_inflow', 'N/A')} / {stock_flow.get('previous_main_net_inflow_pct', 'N/A')}% | 与最新值对照观察变化 |
| 最近3日 / 前序3日净流入 | {stock_flow.get('inflow_3d', 'N/A')} / {stock_flow.get('previous_inflow_3d', 'N/A')} | 观察短期资金变化 |
| 最近3日流入天数 | {stock_flow.get('positive_days_3d', 'N/A')} | 资金结构参考 |
| 5日 / 10日净流入 | {stock_flow.get('inflow_5d', 'N/A')} / {stock_flow.get('inflow_10d', 'N/A')} | 中短期背景参考 |
...
> 这是场内 ETF 二级市场成交资金流，不是一级市场申购/赎回。`as_of` 早于完整日线日期时只作时效限制说明，不能单独改变评分或买卖动作；盘中不得把上一交易日数据表述为今日资金流。
"""
