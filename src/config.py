...
class Config:
    ...
    searxng_public_instances_enabled: bool = False  # Public discovery is opt-in to avoid untrusted-result pollution
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        if not self.has_search_capability_enabled():
            issues.append(ConfigIssue(
                severity="info",
                message="未配置搜索引擎能力 (Anspire/Bocha/MiniMax/Tavily/Brave/SerpAPI/SearXNG)，新闻搜索功能将不可用",
